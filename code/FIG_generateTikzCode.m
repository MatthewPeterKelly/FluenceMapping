function [ ] = FIG_generateTikzCode( R, isStandAlone, showDoseRatePatterns)
...
% Create solnT structure from input
solnT = struct();
nT = length(R);
for iT = 1 : nT
    solnT(iT).T = R{iT}(end).traj.time(end);
    solnT(iT).soln = R{iT};
end
...
objT = zeros(1,numT);
for iT = 1 : numT
    for iter = 1 : length(solnT(iT).soln)
        objT(iT) = objT(iT) + solnT(iT).soln(1,iter).obj;
    end
end
minObj = 0; %min(objT);
maxObj = max(objT);
...
maxF = ceil(max(maxs));
dmax = solnT(1).soln(1,1).dose.rGrid(end);
