...
figNum = 1028;
load(fileName);
hFig = figure(figNum); clf(figNum); 
set(hFig, 'Visible', 'off');
set(hFig,'units','normalized','outerposition',[0 0 1 1]);

iRowSet = [2,3,4,6,10,11,12,13]; % max row is 13
duration = 1:1:9;

% Output directory:
outputDirectory = ['results', filesep, 'cortDataIter_',...
...
[xMin, fMin, countEval, stopFlag, output, bestEver] = cmaes(...
    'leafTrajMultiFitObjIter', zGuess, zSigma, options, ...  % standard arguments
    dose, guess, target, param);  % pass through to objective
finalTime = toc(startTime);

%% Get the best-ever solution:
[objVal, soln] = leafTrajMultiFitObjIter(bestEver.x, dose, guess, target, param);

% Pack up optimization data:
optData.solveTime = finalTime;
optData.countEval = countEval;
optData.stopFlag = stopFlag;
optData.output = output;
optData.bestEver = bestEver;
optData.options = options;

% Output for plotting code:
solnTimeDataStruct(iDuration).T = duration(iDuration);
solnTimeDataStruct(iDuration).solveTime = finalTime;
solnTimeDataStruct(iDuration).soln = soln;
solnTimeDataStruct(iDuration).optData = optData;
...
clf(figNum); 
tGrid = soln{iSoln}(end).traj.time;

subplot(2,2,2); hold on;
plot(tGrid, soln{iSoln}(end).traj.xLow,'r-o');
plot(tGrid, soln{iSoln}(end).traj.xUpp,'b-o');
xlabel('time');
...
plot(tGrid, soln{iSoln}(end).traj.dose, 'g-o');
xlabel('time')
ylabel('fluence dose')
h.YLim = [0, h.YLim(2)];

subplot(2,2,1); hold on;
plot(soln{iSoln}(end).target.fGrid, soln{iSoln}(end).target.xGrid,'rx-')
plot(soln{iSoln}(end).target.fSoln, soln{iSoln}(end).target.xGrid,'ko-','LineWidth',2)
ylabel('position')
xlabel('fluence dose')
