...
\KvAcomment{I'm curious about the start/end position search. I'm using 2 methods: one uses restrictive sampling, the other partial enumeration. Both assume unidirectional trajectories though.}
...
\todo{Include figure} \KvAcomment{Do you already have a figure? I can tweak mine, which is based on the passing time representation. (see figureIntegral.tex)}

\begin{equation}
  g(x) = \int_{t = T_R(x)}^{T_L(x)} d(t) dt
\end{equation}
\noindent where d(t) is the dose rate at time $t$.
\KvAcomment{Are you assuming that the leaves start and end with no gap in between? That's necessary for \ref{opt} to hold.}
